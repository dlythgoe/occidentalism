...
- Tags in `index.json` use a consolidated all-English, lowercase vocabulary (~54 tags). `papers+database/tag-mapping.csv` maps contributors' raw tags (as submitted in their xlsx sheets, which stay untouched) to the site vocabulary — re-apply it when deriving site tags from the database.
